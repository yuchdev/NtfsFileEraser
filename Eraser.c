...
#define CLEANBUFSIZE 65536
	...
	if( !buffersAlloced ) {
		...
		for( i = 0; i < 3; i++ ) {
			...
			case 2:
                ...
                for (j = 0; j < CLEANBUFSIZE; j++) {
                    cleanBuffer[i][j] = (BYTE)rand();
                }
				break;
			}
		}	
	...
	for( passes = 0; passes < NumPasses; passes++ ) {
		...
		if( passes != 0 ) {
            ...
            newFilePointer = SetFilePointer( FileHandle, -seekLength, NULL, FILE_CURRENT );
		}
...
//--------------------------------------------------------------------
VOID SecureDelete( PTCHAR FileName, DWORD FileLengthHi, DWORD FileLengthLo ) 
{
	HANDLE	hFile;
	...
	if( FileLengthLo || FileLengthHi ) {
        ...
        DWORD newFilePointer = SetFilePointer( hFile, FileLengthLo, &FileLengthHi, FILE_BEGIN );
        ...
        newFilePointer = SetFilePointer( hFile, 0, NULL, FILE_BEGIN );
...
VOID ProcessFile( PWIN32_FIND_DATA FindData, TCHAR *FileName )
{
	...
	if( FindData->dwFileAttributes & FILE_ATTRIBUTE_COMPRESSED ||
		...
		// physically on disk.
		if( SecureDeleteCompressed( FileName )) 
            return;
	} 
...
// free space filename information.
//--------------------------------------------------------------------
BOOLEAN CleanFreeSpace( PTCHAR DrivePath )
{
	...
	// Allocate a buffer that is a cluster in size
	...
	if( ZapFreeSpace ) {
		...
		while( 1 ) {
			...
			// Don't close the file, since we want it to keep the space until we're done
		}
	}
...
int _tmain( int argc, TCHAR *argv[] ) 
{
	...
	if( !ZapFreeSpace ) {
		...
		if( !FilesFound ) 
            _tprintf(_T("No files found that match %s.\n"), argv[argc-1] );
